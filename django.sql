...
BEGIN
    ...
    select Stock into pstock from core_product where ID=NEW.ProductID;
--     select Total into carttotal from core_cart where CartID=NEW.CartID;
	if pstock>New.quantity then
